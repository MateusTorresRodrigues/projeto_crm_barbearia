...
export function GradeSemana({
  ...
  function agendamentosDoDia(dia: Date) {
    return agendamentos.filter((a) => mesmodia(new Date(a.data_hora_inicio), dia))
  }
  ...
  function handleDrop(e: React.DragEvent<HTMLDivElement>, dia: Date) {
    e.preventDefault()
  ...
  function handleClickColuna(e: React.MouseEvent<HTMLDivElement>, dia: Date) {
    const rect = e.currentTarget.getBoundingClientRect()
    const y = e.clientY - rect.top
    ...
    <div className="overflow-x-auto rounded-lg border border-border bg-card">
      <div className="flex min-w-[800px]">
        ...
        {semana.map((dia) => {
          ...
          const eventosDoDia = agendamentosDoDia(dia)
          const ehHoje = mesmodia(dia, hoje)

          return (
            <div key={dia.toISOString()} className="flex-1 border-r border-border last:border-r-0">
              <div
                className={cn(
                  "flex h-12 flex-col items-center justify-center border-b border-border text-xs",
                  ehHoje && "bg-primary/10"
                )}
              >
                <span className="text-muted-foreground">{formatarDiaSemanaCurto(dia)}</span>
